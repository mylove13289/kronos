...
def main(config: dict):
    ...
    # 修改create_dataloaders调用，移除分布式采样器
    train_loader, val_loader, train_dataset, valid_dataset = create_dataloaders_cpu(config)

    # 修改训练函数调用，移除DDP相关参数
    _, dt_result = train_model_cpu(
...
def create_dataloaders_cpu(config: dict):
    """
    Creates and returns dataloaders for training and validation (CPU version).
    ...
    Returns:
        tuple: A tuple containing (train_loader, val_loader, train_dataset, valid_dataset).
    """
    print("Creating dataloaders for CPU training...")
    train_dataset = QlibDataset('train')
    valid_dataset = QlibDataset('val')
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        num_workers=config.get('num_workers', 2),
        pin_memory=False,  # CPU上不需要pin_memory
        drop_last=True
    )
    val_loader = DataLoader(
        valid_dataset,
...
def train_model_cpu(model, device, config, save_dir, logger, rank, world_size,
                   train_loader, val_loader, train_dataset, valid_dataset):
    """
    The main training and validation loop for the tokenizer (CPU version).
    ...
    for epoch_idx in range(config['epochs']):
        ...
        if rank == 0:
            ...
            if avg_val_loss < best_val_loss:
                best_val_loss = avg_val_loss
                save_path = f"{save_dir}/checkpoints/best_model"
                model.save_pretrained(save_path)  # 注意：不再使用 model.module
                print(f"Best model saved to {save_path} (Val Loss: {best_val_loss:.4f})")
                if logger:
                    logger.log_model("best_model", save_path)

    dt_result['best_val_loss'] = best_val_loss
    return model, dt_result  # 返回模型和结果



if __name__ == '__main__':
    # Usage: torchrun --standalone --nproc_per_node=4 train_tokenizer.py
