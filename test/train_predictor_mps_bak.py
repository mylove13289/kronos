...
def main(config: dict):
    ...
    # 修改 dataloader 创建方式
    def create_single_dataloaders(config: dict):
        train_dataset = QlibDataset('train')
        valid_dataset = QlibDataset('val')
    ...
    # Start Training
    train_loader, val_loader, train_dataset, valid_dataset = create_single_dataloaders(config)
    ...
    for epoch_idx in range(config['epochs']):
        epoch_start_time = time.time()
        model.train()
        train_dataset.set_epoch_seed(epoch_idx * 10000)  # 移除 rank 参数
        ...
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            save_path = f"{save_dir}/checkpoints/best_model"
            model.save_pretrained(save_path)  # 移除 module 引用
            print(f"Best model saved to {save_path} (Val Loss: {best_val_loss:.4f})")
